...
def getActorDraft(e):
    latestDraftWokitem = [
        wi for wi in e.getListOfWorkitems() if wi.activity_id == "Draft"
    ][-1]
    return latestDraftWokitem.actor


for env in context.Catalog(
    meta_type="Report Envelope",  # noqa: F821
    dataflow_uris=["http://rod.eionet.europa.eu/obligations/602"],
):
    e = env.getObject()
    previous_feedback = [
        x.id
        for x in e.objectValues("Report Feedback")
