...
for c in container.objectValues("Report Collection"):
    for e in container.Catalog(meta_type="Report Envelope", path=c.absolute_url(1)):
        env = e.getObject()
        for w in [
            x for x in env.objectValues("Workitem") if x.activity_id == "AutomaticQA"
        ]:
            try:
                before_time = [
                    x["time"]
                    for x in w.event_log
                    if x["event"] == "assigned to openflow_engine"
                ][0]
                w.addEventOnTime(
                    container.EnvelopeQAApplication.decodeAppData(w, "Automatic QA"),
                    before_time,
                )
                print(w.event_log)
            except:
