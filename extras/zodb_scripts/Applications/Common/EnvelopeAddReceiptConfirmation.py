...
for obl in list(the_envelope.dataflow_uris):
    list_of_obligations.append(
        """<strong>%s</strong>
     (<a href="%s">%s</a>)"""
        % (the_envelope.dataflow_lookup(obl)["TITLE"], obl, obl)
    )
...
<ol>""" % (
    obligations_para,
    the_envelope.getCountryName(),
    DateTime().strftime("%d %B %Y"),
    the_envelope.title_or_id(),
    the_envelope.absolute_url(),
    the_envelope.absolute_url(),
)
...
""" % (
    the_envelope.getLDAPUserCanonicalName(the_envelope.getLDAPUser(getActorDraft())),
    getActorDraft(),
)
