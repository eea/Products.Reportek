...
for du in list(context.getMySelf().dataflow_uris):
    ...
    instance_parent = context.getMySelf().restrictedTraverse(
    ...
    if instance_parent and hasattr(instance_parent, 'instanceFile'):
        ...
        if data_instance:
            context.getMySelf().manage_addDocument(id='questionnaire_%s.xml' % dn,
                                                   title='%s questionnaire' % dt, file=data_instance.data, content_type='text/xml')
