...
l_country = l_country.replace('Kosovo under the UNSCR 1244/99', 'Kosovo (UNSCR 1244/99)')
...
for obl in list(l_envelope.dataflow_uris):
    ...
    if obl == 'http://rod.eionet.europa.eu/obligations/269':
        list_of_obligations.append("""<strong>%s</strong> (<a href="%s">%s</a>)""" % (l_envelope.dataflow_lookup(obl)['TITLE'] + ' (Report on implementation of the measures under Council Directive 92/43/EEC, including main results of surveillance under Article 11 of the Directive)', obl, obl))
    else:
        list_of_obligations.append("""<strong>%s</strong> (<a href="%s">%s</a>)""" % (l_envelope.dataflow_lookup(obl)['TITLE'], obl, obl))
...
documents_list = l_envelope.objectValues(['Report Document', 'Report Hyperlink'])
...
l_title = "Receipt of delivery of the %s %s" % ((' national contribution to' if l_envelope.dataflow_uris == ['http://rod.eionet.europa.eu/obligations/690'] else ''), obligations_title)
l_envelope.manage_addFeedback(title=l_title, feedbacktext=l_ret, automatic=1, content_type='text/html')
