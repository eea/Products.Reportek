...
for obl in list(context.getMySelf().dataflow_uris):  # noqa: F821
    list_of_obligations.append("""%s (<a href="%s">%s</a>)""" % (
        context.getMySelf().dataflow_lookup(obl)['TITLE'],  # noqa: F821
...
<p>In support of INSPIRE – An Infrastructure for Spatial Information in the
 European Community</p>
<br />
<p><strong>To Whom It May Concern</strong></p>

<br />
<p>This confirmation letter shows the status of national submissions to
 Reportnet based on the European Reporting Obligation</p>

<p>INSPIRE Directive: %s</p>

<p>The letter is automatically generated when the authorised national data
 reporter has released the respective information inside the Central Data
  Repository (CDR).</p>

<p>
The following files have been submitted for country <strong>%s</strong> into
 the envelope <a href="%s"><strong>%s</strong></a> and are made publicly
...
<ol>""" % (obligations_para, context.getMySelf().getCountryName(),  # noqa: F821
           context.getMySelf().absolute_url(),  # noqa: F821
           context.getMySelf().title_or_id(), DateTime().strftime('%d %B %Y'))  # noqa: F821
...
for f in documents_list:
    l_ret += '<li>' + f.id + \
        ' (<a href="' + f.absolute_url() + '">' + f.absolute_url() + \
        '</a>)</li>'
...
user_draft = context.getMySelf().getPreviousActor(str(int(workitem_id) - 2))  # noqa: F821
...
<p>%s</p>
""" % (user_draft,
       context.getLDAPUserCanonicalName(context.getLDAPUser(user_draft)),  # noqa: F821
