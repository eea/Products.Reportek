...
for obl in list(context.getMySelf().dataflow_uris):
    list_of_obligations.append("""%s (<a href="%s">%s</a>)""" % (context.getMySelf().dataflow_lookup(obl)['TITLE'], obl.replace('eionet.eu.int','eionet.europa.eu'), obl.replace('eionet.eu.int','eionet.europa.eu')))
...
for f in documents_list:
    l_ret += '<li>' + f.id + ' (<a href="' + f.absolute_url() + '">' + f.absolute_url() + '</a>)</li>'
