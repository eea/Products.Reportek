## Script (Python) "send_mails_username"
...
for row in context.recipients_username():
    recipients = ['BDR.helpdesk@eea.europa.eu','f-gases.reporting@eea.europa.eu']
    contactline = row[4]
...
#   recipients=['soren.roug@eea.europa.eu']
    context.MailHost.send(context.render_message_username(context, context.REQUEST,
                ...
                CONTACT=context.utf8escape(contactline,1),
                CONTACTEMAIL=contactmail),
        recipients, 
      'BDR.helpdesk@eea.europa.eu (BDR Helpdesk)',
     'F-Gases reporting: %s / %s - Account information' % (row[3], row[2]))
return printed
