...
from Products.GenericSetup.utils import XMLAdapterBase, exportObjects, importObjects
...
class LDAPUserFolderXMLAdapter(XMLAdapterBase):
    ...
    def _exportNode(self):
        ...
        self._logger.info(
            "LDAPUserFolder at %s exported." % (self.context.absolute_url_path())
        )
        return node
    ...
    def _importNode(self, node):
        ...
        self._logger.info(
            "LDAPUserFolder at %s imported." % (self.context.absolute_url_path())
        )
    ...
    def _initSettings(self, node):
        ...
        for child in node.childNodes:
            ...
            multivalues = [
                x for x in child.childNodes if x.nodeType == child.ELEMENT_NODE
            ]
    ...
    def _initAdditionalGroups(self, node):
        ...
        value_nodes = [x for x in child.childNodes if x.nodeType == child.ELEMENT_NODE]
    ...
    def _initGroupsStore(self, node):
        ...
        for child in node.childNodes:
            ...
            for grandchild in child.childNodes:
                ...
                values = [
                    x for x in grandchild.childNodes if x.nodeType == child.ELEMENT_NODE
                ]
        ...
        for child in node.childNodes:
            ...
            for grandchild in child.childNodes:
                ...
                self.context.manage_addServer(
                    ...
                    port=port,
                    use_ssl=use_ssl,
                    conn_timeout=int(grandchild.getAttribute("conn_timeout") or 5),
    ...
    def _initLDAPSchema(self, node):
        ...
        for child in node.childNodes:
            ...
            for grandchild in child.childNodes:
                ...
                def get(n):
                    return grandchild.getAttribute(n).encode(self._encoding)
