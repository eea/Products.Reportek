...
from Products.LDAPUserFolder.LDAPUserFolder import (
    LDAPUserFolder,
    manage_addLDAPUserFolder,
)
from Products.Reportek.auth import add_PAS
from update_auth.ldapfolder_migration import exportLDAPUserFolder, importLDAPUserFolder
...
def update_authentication(app):
    ...
    for user_id in local_users:
        ...
        pas["users"].addUser(user_id=user_id, login_name=user_id, password=user["pass"])
        pas["roles"].assignRoleToPrincipal(role_id=user["role"], principal_id=user_id)
