...
def convert(doc, allow_missing):
    ...
    if os.path.exists(fs_path):
        ...
        if os.path.exists(alternate_fs_path):
            ...
            raise ValueError("No data file found on filesystem: %r (+'.undo')", fs_path)
...
def convert_all(
    parent, limit=None, skip=0, allow_missing=False, warnings=True, report=True
):
    ...
    for i, doc in enumerate(iter_documents(parent)):
        ...
        try:
            ...
            total_bytes += size
    msg = ("{path} Migrate documents to blob ({objects} items, {bytes} bytes)").format(
        path=ofs_path(parent), objects=len(out["copied_paths"]), bytes=total_bytes
    )
