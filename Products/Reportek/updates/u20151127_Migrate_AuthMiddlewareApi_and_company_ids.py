...
#   u20151127_Migrate_AuthMiddlewareApi_and_company_ids
#  >>> u20151127_Migrate_AuthMiddlewareApi_and_company_ids.update(app)
...
def migrate_AuthMiddleWareApi(app):
    ...
    if authMiddlewareApi:
        ...
        transaction.commit()
        print(
            "Migration of lockedDownCollections and cleanup of old\
...
def add_company_id_to_collections(app):
    ...
    for path in paths:
        brains = catalog({"path": path, "meta_type": "Report Collection"})
        for brain in brains:
            ...
            if depth >= 4:
                ...
                if "company_id" not in obj.__dict__:
                    ...
                    print(
                        "Added old_company_id: {0}\
                           to collection: {1}".format(obj.getId(), obj.absolute_url())
                    )
                else:
                    ...
                    print(
                        "Added company_id: {0}\
                           to collection: {1}".format(
                            obj.company_id, obj.absolute_url()
                        )
                    )
                obj.reindexObject()
