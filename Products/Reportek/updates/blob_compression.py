...
# >>> from Products.Reportek.updates import blob_compression
# >>> blob_compression.update(app)
...
def update(app, outName="blob_compression.log"):
    ...
    for brain in app.Catalog(meta_type=["Report Document", "File (Blob)"]):
        try:
            ob = brain.getObject()
            # just making sure we populate the aquisition wrapper for later
            # operation on __dict__
            repr(ob)
            ...
            if ob.meta_type == "Report Document":
                # should remove old content_type atribute and let
                # the property take over
            ...
            # new type of object that was created with no compression
            # while solving #20539
            if (
                not old_FileContainer
            ...
            # zip the report doc
            if (
                old_FileContainer or skipped_FileContainer
            ) and data_file._shouldCompress():
