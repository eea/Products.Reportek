        ...
        for brain in brains:
            try:
                ...
                if obj.company_id:
                    ...
                    print(
                        "Created FGAS verification collection for: {}".format(
                            obj.absolute_url()
                        )
                    )
                    logger.info(
                        "Added FGAS verification collection to: %s", obj.absolute_url()
                    )
                    if count % 10000 == 0:
                    ...
                    print(
                        "Skipping non-company collection: {}".format(obj.absolute_url())
                    )
                    logger.info("Not a company collection: %s. ", obj.absolute_url())
