...
def report(app, out):
    ...
    j = {
        "byDocumentType": OrderedDict(
            sorted(
                ((k, v.to_dict()) for k, v in byDocumentType.items()),
                key=sort_by_size,
                reverse=True,
            )
        ),
        "byDocumentTypeMissingBlob": OrderedDict(
            sorted(
                ((k, v.to_dict()) for k, v in byDocumentTypeMissingBlob.items()),
                key=sort_by_size,
                reverse=True,
            )
        ),
        "byBlobType": OrderedDict(
            sorted(
                ((k, v.to_dict()) for k, v in byBlobType.items()),
                key=sort_by_size,
                reverse=True,
            )
        ),
