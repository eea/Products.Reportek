...
def get_posting_date(obj):
    ...
    if obj.id.startswith("AutomaticQA"):
        ...
        logger.info(
            "{} looks like an AutomaticQA feedback, getting\
        ...
        logger.info(
            "{} looks like a manual feedback, getting postingdate\
                     from the last workitem".format(obj.absolute_url())
        )
    else:
        drafts = [wk for wk in obj.getListOfWorkitems() if wk.activity_id == "Draft"]
        last_draft = drafts[-1]
        postingdate = get_last_evtlog_time(last_draft)
        logger.info(
            "{} looks like a conversion log, getting postingdate\
                     from the last draft workitem".format(obj.absolute_url())
        )
    return postingdate
    ...
    for brain in brains:
        ...
        if obj:
            if obj.postingdate != brain.postingdate:
                ...
                logger.info(
                    "Update postingdate for: {} from: {} to: {}.".format(
                        obj.absolute_url(), obj.postingdate, new_postingdate
                    )
                )
