...
# It will basically add i18n tags to prerendered feedback htmls stored in Data.fs
...
# and present cases and cannot cover former, historical versions of such prerendered
# feedback messages.
# This scripts assumes a html layout not more complicated than this:
#  <a_tag attrs="x"> text <inner_tag attrs="x">inner text</inner_tag> text <inner_tag_2 attrs="">inner text 2</inner_tag_2> text </a_tag>
# The msgid is also our input data (taken from feedback specific .pot) and is of form:
#  msgid "text ${var-name-1} text ${var-name-2} text"
# The output is:
#  <a_tag attrs="x" i18n:translate=""> text <inner_tag attrs="x" i18n:name="var-name-1">inner text</inner_tag> text <inner_tag_2 attrs="" i18n:name="var-name-2">inner text 2</inner_tag_2> text </a_tag>
# Please note:
# 1. the input html can be part of a bigger html and is not required to separated from it before input
# 2. any number of inner tags may appear, but not nested!
# 3. If inner html appears they must have a matching var-name-n in the .pot file,
#    otherwise the regex will not match
