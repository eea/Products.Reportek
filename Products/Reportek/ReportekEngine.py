...
from Products.Reportek.constants import DEFAULT_CATALOG, ECAS_ID
from Products.Reportek.ContentRegistryPingger import ContentRegistryPingger
from Products.Reportek.RegistryManagement import (
    BDRRegistryAPI,
    FGASRegistryAPI,
)
...
def _group_ecr_content_data(user_ct):
    ...
    fgas_with_meta = [
        (env, env.get_zope_company_meta()[0]) for env in fgas_audit_envs
    ]
    fgas_grouped = {}
    ...
    return {
        "rw_by_path": _group_by_path(
            rw_colls, lambda k: k.title_or_id().lower()
        ),
        "ro_by_path": _group_by_path(
            ro_colls,
            lambda k: k.getCountryCode() + k.title_or_id().lower(),
        ),
        "audit_by_path": _group_by_path(
            audit_colls, lambda k: k.title_or_id().lower()
        ),
...
class ReportekEngine(Folder, Toolz, DataflowsManager, CountriesManager):
    ...
    def manage_properties(self):
        ...
        if self.auth_middleware_url:
            self.auth_middleware_recheck_interval = int(
                self.REQUEST.get(
        ...
        try:
            self.clamd_timeout = float(
                self.REQUEST.get("clamd_timeout", self.clamd_timeout)
            )
        except (ValueError, TypeError):
            self.clamd_timeout = None
        try:
            self.clam_max_file_size = int(
                self.REQUEST.get("clam_max_file_size", self.clam_max_file_size)
            )
        except (ValueError, TypeError):
            self.clam_max_file_size = None
        self._AVService = AVService(
            self.clamav_rest_host,
            self.clamd_host,
            self.clamd_port,
            self.clamd_timeout,
        ...
        if obl:
            if obl[0] in self.bdr_registry_obligations:
                ...
                for obl in self.dataflow_uris:
                    if (
                        obl in self.er_ods_obligations
                        or obl in self.er_fgas_obligations
                    ):
                        registry = "FGASRegistryAPI"
                        break
                if not getattr(collection, "_company_id", None) and getattr(
                    collection, "old_company_id", None
                ):
                    registry = "BDRRegistryAPI"
    ...
    def create_subcollection(
        ...
        parent_coll.manage_addCollection(
    ...
    def update_company_collection(
        ...
        try:
            ...
            error_msg = "Cannot update collection {0}. Invalid path structure: {1}".format(
                coll_path, str(e)
            )
            logger.warning(error_msg)
            return create_error_response(error_msg)
                    ...
                    envelopeObjects.append(
                        {
                            ...
                            "files": files,
                            "obligation": (
                                obligations[0] if obligations else "Unknown"
                            ),
                            "accepted": accepted,
                        }
                    )
        ...
        for country in ccountries:
            ...
            for brain in brains:
                ...
                for user in users:
                    local_roles = [
                        role
                        for role in doc.get_local_roles_for_userid(user)
                        if role != "Client"
                    ]
            ...
            messages.append({"status": "success", "message": message})
        return messages
    ...
    def sitemap_filter(self, objs):
        return [
            ...
            for x in objs
            if x.meta_type
            in ["Report Collection", "Report Envelope", "Repository Referral"]
        ]
    ...
    def get_apps_wks(self, apps):
        ...
        return catalog(meta_type="Workitem", status="active", activity_id=apps)

    def get_running_envelopes(self):
        catalog = self.unrestrictedTraverse(constants.DEFAULT_CATALOG)
        return catalog(meta_type="Report Envelope", wf_status="forward")
    ...
    def lookup_last_delivery(
        ...
        if l_deliveries:
            ...
            # l_reporting_period
            return RepUtils.utSortByMethod(
                l_deliveries, "getStartDate", l_reporting_period, 1
            )
        return []
    ...
    def migration_table(self):
        ...
        upd_files = [
            f.split(".py")[0]
            for f in os.listdir(upd_path)
            if os.path.isfile("/".join([upd_path, f]))
            and f.startswith("u")
            and len(f.split(".py")) > 1
        ]
    ...
    def manage_editUNSInterface(
        ...
        UNS_username,
        UNS_password,
        UNS_password_confirmation,
        UNS_channel_id,
        UNS_notification_types,
        REQUEST=None,
    ):
    ...
    def get_ecas_user(self, username):
        ...
        try:
            ecas = self.unrestrictedTraverse(ecas_path, None)
            if ecas:
                return ecas.getEcasUser(username)
        except Exception:
            logger.info("Unable to get ecas info")
    ...
    def sendNotificationToUNS(
        ...
        try:
            ...
            logger.warning(
                "Unable to send UNS notification for: {0}: {1}".format(
                    envelope.absolute_url(), e
                )
            )
            res = 0
        return res
        ...
        for act in actors:
            try:
                if l_server is not None:
                    ...
                    l_server.UNSService.makeSubscription(
                        self.UNS_channel_id, act, filters
                    )
                    return 1
            except Exception as e:
                logger.warning(
    ...
    def find_col_with_different_id(self, parent, metadata):
        ...
        relevant = ["country", "dataflow_uris", "locality", "parent"]
        result = None
