...
class BdrAuthorizationMiddleware(Cacheable):
    ...
    def updateLocalRoles(self, user):
        ...
        try:
            # flush everything so far; we do this especially because our caller has some data stored in session
            # and that session will be reset by the next 'begin' if not commited
            transaction.commit()
            transaction.begin()
            # call SatelliteRegistry to get companies for this user, as well as the former companies for him
            ...
            if accessiblePaths is None:
                ...
                raise ValueError("Interrogating Bdr Middleware failed."
                                 ...
                                 % user)
            #unaccessiblePaths = set(['cars/colvgok8a/colvgx9mw'])
            #unaccessiblePaths = set(['cars/colvgok8a/colvgolbg'])
