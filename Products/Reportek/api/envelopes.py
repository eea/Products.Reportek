...
class EnvelopesAPI(BrowserView):
    ...
    def get_files(self, env_path=None):
        ...
        documents = []
        errors = []
        documents_data = {"documents": documents, "errors": errors}
        if env_path:
            ...
            if envelope:
                for doc in envelope.objectValues("Report Document"):
                    ...
                    try:
                        ...
                        errors.append({"title": t, "description": str(e)})
                    doc_properties = {
    ...
    def get_dates_range_query(self, value, **kwargs):
        ...
        value = {"query": d_query, "range": d_range}
        return value
    ...
    def build_catalog_query(self, valid_filters, fed_params):
        ...
        for param in valid_filters:
            if fed_params.get(param):
                ...
                if get_value:
                    query[c_idx] = get_value(
                        value, param=param, query=query, c_idx=c_idx
                    )
    ...
    def is_filtered_out(self, default_props, additional_filters, fed_params):
        ...
        sdatesrange = (
            "statusDateStart" in additional_filters
            or "statusDateEnd" in additional_filters
            or "activityDateStart" in additional_filters
            or "activityDateEnd" in additional_filters
        )
        if sdatesrange:
            sds = None
            sde = None
        ...
        for afilter in additional_filters:
            ...
            if afilter_v and afilter not in [
                "statusDateStart",
                "statusDateEnd",
                "activityDateStart",
                "activityDateEnd",
    ...
    def get_default_props(self, brain):
        ...
        obls = [obl.split(DF_URL_PREFIX)[-1] for obl in brain.dataflow_uris]
    ...
    def get_envelopes(self):
        ...
        if query:
            ...
            if len(brains) > self.MAX_RESULTS:
                ...
                additional_filters = [
                    k
                    for k in list(self.AVAILABLE_FILTERS.keys())
                ...
                for brain in brains:
                    ...
                    if self.is_filtered_out(
                        default_props, additional_filters, fed_params
                    ):
                        continue
