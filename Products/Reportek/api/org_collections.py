...
class OrgCollections(BrowserView):
    ...
    def create_organisation_folder(self):
        ...
        if not dflow_coll:
            return json.dumps({"success": False, "error": "obligation folder missing"})

        country = dict(dflow_coll.objectItems()).get(country_code)
        if country is None:
            return json.dumps({"success": False, "error": "country folder missing"})
        ...
        return json.dumps({"success": True, "created": created, "path": path})
