...
class BaseDelivery(ReportekContent):
        ...
        if getSecurityManager().checkPermission("View", self):
            if format == "xls":
                ...
                env_data = {
                    ...
                    else company.decode("utf-8"),
                    "userid": userid,
                    "title": self.title
                    if isinstance(self.title, str)
                    else self.title.decode("utf-8"),
                    "id": self.id,
    ...
    def rdf(self, REQUEST):
        ...
        res_a(
            '<Delivery rdf:about="%s">'
            % xmlEncode(parse_uri(self.absolute_url(), http_res))
        )
        ...
        for flow in self.dataflow_uris:
            res.append(
                '<obligation rdf:resource="%s"/>'
                % xmlEncode(flow.replace("eionet.eu.int", "eionet.europa.eu"))
            )
