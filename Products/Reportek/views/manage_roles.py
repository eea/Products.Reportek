...
class ManageRoles(BaseAdmin):
        ...
        if results:
            ...
            if getattr(engine, "col_role_sync_rmq", False):
                for url in [res.get("url") for res in results]:
                    ...
                    queue_msg(url, queue="collections_sync")
    ...
    def revoke_roles(self):
        ...
        for collection in collections:
            ...
            for r in revoke_roles:
                if REPORTEK_DEPLOYMENT == DEPLOYMENT_BDR and r == "Reporter (Owner)":
                    roles.remove("Owner")
                else:
                    roles.remove(r)
            ...
            if sync_transfers:
                ...
                if transfer:
                    ...
                    revocable = [
                        r
                        for r in revoke_roles
                        if r in ["ClientFG", "ClientODS", "ClientCARS", "ClientHDV"]
                    ]
    ...
    def search_ecas_users(self, term):
        ...
        if ecas_db:
            for user in list(ecas_db.values()):
                ...
                if username:
                    if term in username:
                        result = {"uid": username, "mail": email}
                        users.append(result)
                        continue
                if email:
                    if term in email:
    ...
    def search_entities(self):
        ...
        if term:
            if s_type == "groups":
                ...
                if ldap_users:
                    err_users = [
                        user for user in ldap_users if user.get("sn") == "Error"
                    ]
                    if err_users:
    ...
    def display_confirmation(self):
        return (
            self.request.get("username", None) or self.request.get("groupsname", None)
        ) and self.request.get("role", None)
        ...
        for brain in brains:
            ...
            if local_defined_users:
                for entity in local_defined_users:
                    entry = (brain.getPath(), brain.local_defined_roles.get(entity))
                    if entity in disabled_uids:
                        if entity not in result:
                            result[entity] = {"type": "User", "paths": [entry]}
                        else:
                            result[entity]["paths"].append(entry)
                    elif entity.startswith(group_prefixes):
                        if entity not in groups:
                            if entity not in result:
                                result[entity] = {"type": "Group", "paths": [entry]}
                            else:
                                result[entity]["paths"].append(entry)
