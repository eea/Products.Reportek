...
class OpenFlowEngine(Folder, Toolz):
    ...
    manage_addApplicationForm = PageTemplateFile(
        "zpt/Workflow/application_add", globals()
    )

    security.declareProtected("Manage OpenFlow", "manage_editApplicationForm")
    manage_editApplicationForm = PageTemplateFile(
    ...
    def editActivitiesPushableOnRole(
        ...
        if (
            role in self._activitiesPushableOnRole
            and process in self._activitiesPushableOnRole[role]
        ):
        ...
        self._p_changed = 1
        if REQUEST:
            REQUEST.RESPONSE.redirect("Roles")
    ...
    security.declareProtected(
        "Manage OpenFlow", "deleteProcessWithActivitiesPushableOnRole"
    )
    ...
    def editActivitiesPullableOnRole(
        ...
        if activities is None:
            activities = []
        if (
            role in self._activitiesPullableOnRole
            and process in self._activitiesPullableOnRole[role]
        ):
    ...
    def setRestrictionsOnRole(
        self, process, permission, roles, acquire=0, REQUEST=None
    ):
        ...
        for pr in procs:
            ...
            for act in pr.objectValues("Activity"):
                activity = {
                    ...
                    "pushable_roles": [
                        pushR for pushR in self.getPushRoles(pr.id, act.id)
                    ],
                    "pullable_roles": [
                        pullR for pullR in self.getPullRoles(pr.id, act.id)
                    ],
                }
    ...
    def importFromJson(self, file, REQUEST=None):
        ...
        try:
            ...
            if "Invalid rid" in e.args[0]:
                message = (
                    "Failed to import. Id %s is invalid or\
                            already exists."
                    % e.args[1]
                )
            else:
                message = "Failed to import. Is your json file the result\
        ...
        if problem_apps:
            ...
            for app in problem_apps:
                ...
                logger.warning(
                    "Workflow Import/Export: App %s with path:\
                                %s is %s%s"
                    % (
                        app["name"],
                        app["path"],
    ...
    workflow_map_processes = PageTemplateFile(
        "zpt/Workflow/workflowMapProcesses", globals()
    )

    security.declareProtected("Manage OpenFlow", "workflow_map_process")
    workflow_map_process = PageTemplateFile(
...
def handle_application_move_events(obj):
    ...
    if valid_old_ids:
        if obj.oldName not in valid_old_ids:
            messages.append(
                "Id %s was not mapped by path to any activity." % (obj.oldName)
            )
        else:
            messages.append(
