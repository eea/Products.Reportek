...
class DFMTestCase(BaseUnitTest):
    ...
    def add_mapping(self, oid, *args, **kwargs):
        ob = DataflowMappingsRecord(oid, title=args[0], dataflow_uri=args[1])
    ...
    def test_add_schema(self):
        ...
        self.assertTrue(hasattr(self.mappings, "test"))
        request = Mock(form=dict(schema=schema2, name="schema2"))
    ...
    def test_edit_add(self):
        ...
        request = Mock(method="POST", form=dict(add=True))
    ...
    def test_edit_delete(self):
        ...
        request = Mock(method="POST", form=dict(delete=True))
        self.mappings.test.edit(request)
