...
class MigratedObligationsTestCase(BaseTest):
    ...
    def test_table_renders(self):
        ...
        self.assertIn('<select id="seasonal-obligations"', html)
        select = html.split('<select id="migrated-obligations"')[1].split("</select>")[
            0
        ]
        self.assertIn('name="dataflow_uris:list"', select)
        self.assertIn('multiple="multiple"', select)
    ...
    def test_flagging_through_the_form(self):
        ...
        self.app.REQUEST.form.update(
            {"remove": "Reopen selected", "uris": [BASEL, LCP]}
        )
        self.engine.obligation_locks_table()
        self.assertEqual(dict(self.engine.locks), {})
    ...
    def test_add_envelope_form_explains_the_migration(self):
        ...
        self.assertIn("now reported on another platform", html)
        self.assertIn("Yearly report to the Basel Convention", html)
        self.assertIn("https://new.example/obl/8", html)
        self.assertIn("Moved.", html)
...
class FrozenEnvelopeTestCase(BaseTest):
    ...
    def test_overview_explains_the_migration(self):
        ...
        self.assertIn("https://new.example/obl/8", html)
        self.assertIn("Moved.", html)
        # the box needs the strong, that is what the stylesheet hangs its
        # icon off and what fills the gutter it reserves on the left
        banner = html.split('id="obligation-lock"')[1].split("</div>")[0]
        self.assertIn("<strong", banner)
        # a manager still sees the workflow controls, so say why
        self.assertIn("It stays open to you", banner)
...
class LockMigrationTestCase(BaseTest):
    """The update carrying the old migrated flags into the lock store"""
    ...
    def afterSetUp(self):
        ...
        old = OOBTree()
        old[BASEL] = PersistentMapping(
            {
                "target_url": "https://reportnet.europa.eu/",
                "message": "Migrated to Reportnet 3",
                "set_by": "someone",
                "set_on": DateTime("2026/09/17"),
            }
        )
        self.engine._migrated_obligations = old

    def migrate(self):
        """Run the update without committing.

        BaseTest wires the engine up with Mocks, which the ZODB cannot
        pickle, so the commit the update ends with has to sit this out.
        """
        from unittest.mock import patch

        from Products.Reportek.updates import u20260921_obligation_locks

        with patch.object(u20260921_obligation_locks, "transaction"):
            return u20260921_obligation_locks.migrate_locks(self.app)

    def test_flags_become_migrated_locks(self):
        self.migrate()

        record = self.engine.locks[BASEL]
        self.assertEqual(record["kind"], "migrated")
        self.assertEqual(record["strength"], "hard")
        self.assertEqual(record["target_url"], "https://reportnet.europa.eu/")
        # the flag called it a message, a lock calls it a reason
        self.assertEqual(record["reason"], "Migrated to Reportnet 3")
        # the audit trail of the original flag is kept
        self.assertEqual(record["set_by"], "someone")
        self.assertEqual(record["set_on"], DateTime("2026/09/17"))
        # and it behaves like any other migrated lock
        self.assertTrue(self.engine.lock_is_closed(record))
        self.assertFalse(self.engine._migrated_obligations)

    def test_running_it_twice_changes_nothing(self):
        self.migrate()
        before = dict(self.engine.locks[BASEL])
        self.migrate()
        self.assertEqual(dict(self.engine.locks[BASEL]), before)
