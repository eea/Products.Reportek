...
class CatalogTest(BaseTest, ConfigureReportek):
    ...
    def test_envelope_indexes(self, mock_commit):
        ...
        definitions = [
            ...
            ({'meta_type': 'Report Envelope',
              'getCountryName': 'FirstCountry'}, [first_envelope]),
            ({'meta_type': 'Report Envelope', 'years': [2012]}, [
             first_envelope, second_envelope]),
            ({'meta_type': 'Report Envelope',
              'partofyear': 'JANUARY'}, [first_envelope]),
            ({'meta_type': 'Report Envelope', 'process_path': '/ProcessURL'},
             [first_envelope, second_envelope]),
            ({'meta_type': 'Report Envelope',
              'released': 1}, [first_envelope]),
            ({'meta_type': 'Report Envelope',
              'path': '/ReportekEngine/first_envelope'}, [first_envelope]),
        ]
    ...
    def test_document_indexes(self):
        ...
        definitions = [
            ({'meta_type': 'Report Document',
              'xml_schema_location': 'http://example.com/schema'}, [document]),
            ({'meta_type': 'Report Document',
              'content_type': 'application/octet-stream'}, [document]),
        ]
