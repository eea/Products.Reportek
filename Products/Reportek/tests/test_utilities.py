...
SCRIPTS = [
    {"script_id": "buttons_loginout", "filename": "buttons_loginout.py"},
    {"script_id": "buttons_py", "filename": "buttons_py.py"},
]
...
class BaseFunctionalTestCase(ztc.FunctionalTestCase, BaseTest):
    ...
    def _setupFILES(self):
        for file in FILES:
            file_path = os.path.join(
                os.path.dirname(__file__), "data", file.get("filename")
            )
    ...
    def _setupTMPLS(self):
        for tmpl in TEMPLATES:
            tmpl_path = os.path.join(
                os.path.dirname(__file__), "data", tmpl.get("filename")
            )
    ...
    def _setup_users(self):
        ...
        self.app.acl_users.userFolderAddUser(
            user_name, user_password, ["Manager", "Reporter"], []
        )
    ...
    def afterSetUp(self):
        ...
        self.app.manage_addProperty("management_page_charset", "utf-8", "string")
        self._setupSCRIPTS()
        self._setupFILES()
        self._setupTMPLS()
        ...
        wf_engine.setProcessMappings("process", "1", "1", p_dataflows, p_countries)
    ...
    def test_reportek_utilities(self):
        ...
        self.assertTrue(
            "Yearly report to the Fictive Convention" in self.browser.contents
        )
        ...
        if REPORTEK_DEPLOYMENT != DEPLOYMENT_BDR:
            ...
            self._check_controls(self.browser.contents)
            self.assertTrue(
                "Operations completed succesfully." in self.browser.contents
            )
            ...
            self.browser.getLink(
                text="Collections allocated to the wrong country"
            ).click()
            self._check_controls(self.browser.contents)
            self.assertTrue(
            ...
            self.browser.getLink(
                text="Envelopes allocated to the wrong country"
            ).click()
            self.assertTrue(
            ...
            # Test with end date
            self.browser.getControl(name="enddate").value = min_date.strftime(
                "%Y-%m-%d"
            )
            self.browser.getControl(name="btn.search").click()
            self.assertTrue("No envelopes." in self.browser.contents)
            # Test with start date
            self.browser.getControl(name="startdate").value = min_date.strftime(
                "%Y-%m-%d"
            )
            self.browser.getControl(name="btn.search").click()
            self.assertTrue("Test envelope" in self.browser.contents)
            ...
            self.browser.getControl(name="enddate").value = max_date.strftime(
                "%Y-%m-%d"
            )
            self.browser.getControl(name="btn.search").click()
            self.assertTrue("Test envelope" in self.browser.contents)
            ...
            self.browser.getControl(name="enddate").value = mid_date.strftime(
                "%Y-%m-%d"
            )
            self.browser.getControl(name="btn.search").click()
            self.assertTrue("No envelopes." in self.browser.contents)
        ...
        self._check_controls(self.browser.contents)
        self.assertEqual(ru_url + "/@@revoke_roles", self.browser.url)
        ...
        self._check_controls(self.browser.contents)
        self.assertTrue("Roles removed: " in self.browser.contents)
