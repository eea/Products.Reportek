...
class TestThreadSafeKeyManagerProxy(unittest.TestCase):
    ...
    def test_setitem_delegates(self):
        ...
        proxy["_forms"] = "new_ring"
        mgr.__setitem__.assert_called_once_with("_forms", "new_ring")

    def test_getitem_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.__getitem__ = Mock(return_value="secret_value")
        result = proxy["_forms"]
        mgr.__getitem__.assert_called_once_with("_forms")
        self.assertEqual(result, "secret_value")

    def test_contains_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.__contains__ = Mock(return_value=True)
        result = "_forms" in proxy
        mgr.__contains__.assert_called_once_with("_forms")
        self.assertTrue(result)

    def test_iter_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.__iter__ = Mock(return_value=iter(["_forms", "_system"]))
        result = list(proxy)
        self.assertEqual(result, ["_forms", "_system"])

    def test_get_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.get = Mock(return_value="val")
        result = proxy.get("_forms", "default")
        mgr.get.assert_called_once_with("_forms", "default")
        self.assertEqual(result, "val")

    def test_get_with_default(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.get = Mock(return_value=None)
        result = proxy.get("nonexistent", None)
        mgr.get.assert_called_once_with("nonexistent", None)
        self.assertIsNone(result)

    def test_keys_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.keys = Mock(return_value=["_forms", "_system"])
        result = proxy.keys()
    ...
    def test_clear_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        proxy.clear("_system")
        mgr.clear.assert_called_once_with("_system")
    ...
    def test_rotate_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        proxy.rotate("_forms")
        mgr.rotate.assert_called_once_with("_forms")

    def test_rotate_default_ring(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        proxy.rotate()
        mgr.rotate.assert_called_once_with("_system")

    def test_secret_delegates(self):
        proxy, mgr = self._make_proxy_with_mock_manager()
        mgr.secret = Mock(return_value="the_secret")
        result = proxy.secret("_forms")
        mgr.secret.assert_called_once_with("_forms")
    ...
    def test_secret_default_ring(self):
        ...
        result = proxy.secret()
        mgr.secret.assert_called_once_with("_system")
    ...
    def test_resolves_key_manager_from_request(self, mock_getRequest, mock_getRoot):
        mock_request = Mock()
    ...
    @patch("plone.protect.utils.getRoot")
    @patch("zope.globalrequest.getRequest")
    def test_falls_back_to_underscore_key_manager(self, mock_getRequest, mock_getRoot):
    ...
    @patch("plone.protect.utils.getRoot", return_value=None)
    @patch("zope.globalrequest.getRequest")
    def test_parents_fallback_uses_last_parent(self, mock_getRequest, mock_getRoot):
