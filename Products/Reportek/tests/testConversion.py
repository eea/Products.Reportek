...
class FundamentalsTestCase(BaseTest):
    ...
    def test_absolute_url(self):
        ...
        self.assertEqual("http://nohost", self.app.absolute_url())
...
class ConvertersTestCase(BaseTest, ConfigureReportek):
    ...
    def test_nullSuffixConverter(self, mock_http_params):
        ...
        mock_http_params.return_value = []
        converters = getattr(self.app, CONVERTERS_ID)
        converters.manage_addConverter(
            "pdftext",
            title="Show as text",
            convert_url="pdf2txt %s",
            ct_input="application/pdf",
            ct_output="text/plain",
            suffix="",
        )
    ...
    @patch.object(Converters, "_http_params")
    @patch("Products.Reportek.Converters.xmlrpc.client")
    def test_suffixXmlConverter(self, mock_xmlrpc_client, mock_http_params):
        ...
        server = mock_xmlrpc_client.ServerProxy.return_value
    ...
    @patch.object(Converters, "_http_params")
    @patch("Products.Reportek.Converters.xmlrpc.client")
    def test_gmlConverter(self, mock_xmlrpc_client, mock_http_params):
    ...
    @patch.object(Converters, "_get_remote_converters", return_value=[])
    @patch.object(Converters, "_http_params")
    def test_xml_converters_without_schema_accepted(
        self, mock_http_params, mock_get_remote_converters
    ):
    ...
    @patch.object(Converters, "_get_remote_converters", return_value=[])
    @patch.object(Converters, "_http_params")
    def test_xml_converters_with_bad_schema_rejected(
        self, mock_http_params, mock_get_remote_converters
    ):
