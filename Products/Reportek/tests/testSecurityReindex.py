...
from Products.Reportek import security_reindex
...
class ReindexSecurityBatchedTest(BaseTest, ConfigureReportek):
    ...
    def _add_envelopes(self, n):
        ...
        RepUtils.generate_id = lambda template: "%s_%d" % (
            template, next(ids))
        try:
            envs = []
            for i in range(n):
    ...
    def test_walker_visits_all_descendants(self):
        ...
        expected = sum(
            1 for brain
            in catalog.unrestrictedSearchResults(path=col_path)
        ...
        visited = {"/".join(c[0][0].getPhysicalPath())
        ...
        for call in spy.call_args_list:
            kwargs = call[1]
            self.assertEqual(kwargs.get("idxs"),
                             security_reindex.SECURITY_INDEXES)
    ...
    def test_walker_skips_self(self):
        ...
        for call in spy.call_args_list:
            target = call[0][0]
            self.assertNotEqual(
                "/".join(target.getPhysicalPath()), col_path,
...
class OnLocalRolesChangedSubscriberTest(BaseTest, ConfigureReportek):
    ...
    def test_bdr_runs_cascade_unconditionally(self):
        ...
        with patch.object(
            ...
            security_reindex, "reindex_security_batched", return_value=7,
        ) as mock_walker:
            security_reindex.on_local_roles_changed(
                self.col,
                LocalRolesChangedEvent(self.col, {"AnyRole"}))
...
class CollectionManageRolesIntegrationTest(BaseTest, ConfigureReportek):
    ...
    def test_set_local_roles_without_request_does_not_fire_event(self):
        ...
        self.assertIn(
            "Reporter", self.col.get_local_roles_for_userid("alice"))
    ...
    def test_bdr_del_local_roles_with_request_fires_event_with_old_roles(self):
        ...
        evt = self.captured[0]
        self.assertEqual(
            evt.changed_roles, frozenset({"RoleA", "RoleB"}))
        ...
        try:
            for i in range(3):
                self.create_envelope(
                    self.col,
                    title="Env%d" % i, year=2012, endyear=2013,
                    country="http://spatial/1", locality="", descr="",
                )
        finally:
            RepUtils.generate_id = original_generate_id
    ...
    def test_bdr_zmi_runs_walker_for_any_role(self):
        ...
        with patch(
            ...
            self.col.manage_permission(
                "View", roles=["RealViewer"], acquire=0)
            self.col.manage_permission(
