...
class ContentRegistryPinggerTest(BaseTest, ConfigureReportek):
    ...
    def test_ping_many_async(self):
        ...
        self.pingger.content_registry_ping_async(
            [uri1, uri2], ping_argument="create", envPathName=envName
        )
        mysleep(0.05)
...
class InitCRTest(BaseTest, ConfigureReportek):
    ...
    if REPORTEK_DEPLOYMENT == DEPLOYMENT_CDR:
        ...
        def test_ping_remaining_envelopes(self, redis_mock):
            ...
            val = pickle.dumps(val)
            self.pingger.PING_STORE.hset(
                constants.PING_ENVELOPES_REDIS_KEY,
                self.envelope.absolute_url(),
                val,
            )
            val = {"op": None, "ts": 0}
            val = pickle.dumps(val)
            self.pingger.PING_STORE.hset(
                constants.PING_ENVELOPES_REDIS_KEY,
                self.second_envelope.absolute_url(),
                val,
            )
            ok_message = """<?xml version="1.0"?>
            <response>
            ...
            self.assertIn(
                call(
                    self.envelope.absolute_url() + "/link",
                    ping_argument="create",
                ),
                call_args_list,
            )
            self.assertIn(
                call(
                    self.envelope.absolute_url() + "/foo.txt",
                    ping_argument="create",
                ),
                call_args_list,
            )
            self.assertNotIn(
                call(
                    self.second_envelope.absolute_url() + "/rdf",
                    ping_argument="create",
                ),
                call_args_list,
            )
