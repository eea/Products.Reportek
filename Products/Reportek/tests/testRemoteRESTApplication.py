...
class RemoteRESTApplicationProduct(WorkflowTestCase):
    def setUp(self):
        ...
        self.apps_folder._setOb("Common", Folder("Common"))
    ...
    def test_request_for_new_job_success(self, mock_requests):
        ...
        mock_requests.get.assert_called_once_with(
            "http://submit.url/",
            params={"EnvelopeURL": "http://nohost/col1/env1", "f": "pjson"},
        )
    ...
    def test_add_application_to_zope_object(self):
        ...
        self.assertEqual(
            "http://submit.url/",
            self.apps_folder.Common.restapp.ServiceSubmitURL,
        )
        self.assertEqual(
            "http://check.url/",
            self.apps_folder.Common.restapp.ServiceCheckURL,
        )
        # asser defaults
    ...
    def test_app_writes_success_in_event_log(self, mock_requests):
        ...
        self.create_cepaa_set(1)
        exp = re.compile(
            r"\w+ job request for http:\/\/[\w+\/]+ successfully submited.$"
        )
    ...
    def test_request_for_new_job_invalid_status_code(self, mock_requests):
        ...
        self.create_cepaa_set(1)
        exp = re.compile(
            r"\w+ job request for http:\/\/[\w+\/]+ returned invalid status code 201.$"  # noqa
        )  # noqa
        self.assertRegex(self.app.col1.env1["0"].event_log[3]["event"], exp)
    ...
    def test_job_invalid_status_code(self, mock_requests):
        ...
        exp = re.compile(
            r"\w+ job id 1 for http:\/\/[\w+\/]+ returned invalid status code 201.$"  # noqa
        )  # noqa
        self.assertRegex(self.app.col1.env1["0"].event_log[-3]["event"], exp)
    ...
    @patch("Products.Reportek.RemoteRESTApplication.requests")
    def test_job_success_attaches_zipfile_to_feedback(
        self, mock_requests, mock_local_converters
    ):
        mock_requests.get.return_value = Mock(
            status_code=200,
            json=Mock(return_value={"jobId": 1, "jobStatus": "esriJobSubmitted"}),
            ...
            Mock(
                ...
                json=Mock(
                    return_value={
                        "paramName": "ResultZip",
                        "dataType": "GPString",
                        "value": "http://test/\\server\\job\\scratch/result.zip",  # noqa
                    }
                ),
            ),
            Mock(
