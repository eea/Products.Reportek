...
from Products.Reportek.XMLInfoParser import (
    detect_schema,
...
def xml_bytes(content):
    """Helper to convert XML string content to BytesIO for lxml parsing."""
...
class XmlDetectionTest(BaseUnitTest):
    def test_create_xml_document(self):
        ...
        schema_location = detect_schema(xml_bytes(content))
        self.assertEqual(
            schema_location,
    ...
    def test_bad_xml_document(self):
        ...
        exception_args = None
        try:
            detect_schema(xml_bytes(content))
    ...
    def test_create_xml_good_multiple_schemaLocation(self):
        ...
        self.assertEqual(
            schema_location,
            "https://schema.eu/schema.xsd https://schema.eu/schema1.xsd",
        )
    ...
    """
        ...
        self.assertEqual(
            schema_location,
            "http://dd.eionet.europa.eu/GetSchema?id=TBL1927 http://dd.eionet.europa.eu/GetSchema?id=TBL2000",
        )

    def test_dtd_public_id(self):
        content = (
            '<!DOCTYPE zz PUBLIC "-//some//public//doctype" '
...
class XmlSingleSchemaDetectionTest(BaseUnitTest):
    def test_single_schema_no_ns(self):
        ...
        schema_location = detect_single_schema(xml_bytes(content))
