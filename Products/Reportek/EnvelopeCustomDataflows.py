...
from . import RepUtils, zip_content
...
class EnvelopeCustomDataflows(Toolz):
    ...
    def log_file_conversion(self, filename, result):
        ...
        if hasattr(self, "REQUEST"):
            for l_w in self.getWorkitemsActiveForMe(self.REQUEST):
                l_w.addEvent(
                    "file conversion",
                    "File: {0} ; ({1})".format(filename, result),
                )
    ...
    def convert_excel_file(
        ...
        self.manage_addDocument(
            id=l_id,
            title=l_original_type,
            file=l_original_content,
        ...
        try:
            ...
            t_docs = self.objectValues("Report Document")
            dt_ids = [
                x.id
                for x in t_docs
            ...
            if l_result in ["0", "1"]:
                ...
                if REQUEST is not None:
                    if len(l_converted_files) == 0:
                        ...
                        l_msg = (
                            """The file was successfully uploaded in """
                ...
                if REQUEST is not None:
                    ...
                    l_err = (
                        """[ERROR]: Conversion failed due to a system """
                ...
                if REQUEST is not None:
                    ...
                    self.log_file_conversion(l_id, l_err)
                    msg = (
                        """Incorrect result from the Conversion service. """
                        """The file was successfully uploaded in the """
                        """envelope, but not converted into an XML """
                        """delivery."""
                    )
    ...
    def manage_addDDzipfile(
        ...
        if not isinstance(file, str) and hasattr(file, "filename"):
            ...
            xml_f = {}
            docs = [
                (x.xml_schema_location, x.id)
                for x in self.objectValues("Report Document")
                if x.xml_schema_location
            ]
            for i, j in docs:
                if i in xml_f:
                    xml_f[i].append(j)
            ...
            if not files_only:
                ...
                if REQUEST is not None:
                    return error_message(
                        self, msg, action="index_html", REQUEST=REQUEST
                    )
                else:
                    if verbose:
                        return 0, msg
            ...
            for name in zip_file_ids:
                ...
                if re.search(self.fileTypeByName_pattern, id):
                    if not schemaless_shp_meta and (
                        required_schema
                        and zipped_file.xml_schema_location
                        not in RepUtils.utConvertLinesToList(required_schema)
                    ):
                    ...
                    if replace_xml and zipped_file.xml_schema_location in list(
                        xml_f.keys()
                    ):
                        # delete all the XML files from this envelope which
                        # have this schema
            ...
            if file_ids_not_uploaded:
                if len(file_ids_not_uploaded) == len(zip_file_ids):
                    msg = (
                        """No files were added in the envelope, because """
            ...
            if REQUEST is not None:
                if failed:
                    return error_message(
                        self, msg, action="./manage_main", REQUEST=REQUEST
                    )
                else:
                    errors = []
                    if not msg.startswith("Files successfully"):
                        err = {"title": "WARNING", "description": msg}
                        errors.append(err)
                    return success_message(
                        self,
                        objs,
        ...
        if engine.UNS_server:
            ...
            for df in self.dataflow_uris:
                if event_type:
                    filters.append(
                        {
                            "http://rod.eionet.europa.eu/schema.rdf#locality": country_name,  # noqa
                            "http://rod.eionet.europa.eu/schema.rdf#obligation": engine.getDataflowTitle(  # noqa
                                df
                            ),  # noqa
    ...
    def uploadGISfiles(
        ...
        """ """
        if (
            file_shp.filename.find(".shp") != -1
            and file_shx.filename.find(".shx") != -1
            and file_prj.filename.find(".prj") != -1
            and file_dbf.filename.find(".dbf") != -1
            and file_metainfo.filename.find(".xml") != -1
        ):
            objs = []
            ...
            file_shp.filename
            and file_shx.filename
            and file_prj.filename
            and file_dbf.filename
    ...
    def uploadGISZIPfiles(self, file_gis_zip=None, REQUEST=None):
        ...
        if file_gis_zip.filename:
            ...
            for name in l_file_list:
                ...
                if name[-1] == "/" or name[-1] == "\\":
                    l_mess = (
                        """Files not uploaded! The zip file you """
            ...
            if l_mess:
                if REQUEST is not None:
                    return error_message(
                        self, l_mess, action="./index_html", REQUEST=REQUEST
                    )
                else:
                    return 0
    ...
    def createRegionInstances(
        self, x="3", y="28", form_name="form2.xml", language="en"
    ):
    ...
    def getZonesForRegion(self, form_name=None, aqq_rule=None):
        ...
        will furthermore filter the data according to the chemical compound
        being edited.
        @param aqq rule to further filter the zones, see b) from ticket.
        These are hard-coded conventions, for example 'form8b' refers to form
        8, second tab
        If no rule is specified, defaults to True, including the zone within
        the result
        ...
        for xml_file in self.objectValues("Report Document"):
            ...
            if schema in list(schema_convs.keys()):
                ...
                try:
                    ...
                    do_log(
                        "info",
                        "Successfully sanitized: {}.".format(xml_id),
                        wk,
                    )
                ...
                do_log(
                    ...
                    """the required schema(s).""".format(xml_file.getId()),
                    wk,
                )

    security.declareProtected("Change Envelopes", "manage_addMMRXLSFile")

    def manage_addMMRXLSFile(
        self,
        file="",
        restricted="",
        required_schema=[],
        ...
        if colls:
            ...
            for k in list(envs.keys()):
                c_colls = (
                    ...
                    if getSecurityManager().checkPermission("View", col)
                )
                for col in c_colls:
                    if col.company_id == self.company_id:
                        envs[k] = envs[k] + [
                            env for env in col.objectValues("Report Envelope")
                        ]
        ...
        try:
            ...
            for f_id in files_to_delete:
                if not isinstance(f_id, str):
                    err = "Invalid item in 'files' list: all items must be strings."
                    raise ValueError(err)
    ...
    def unassign_auditor(self, notify_event=True):
        ...
        try:
            ...
            if res.get("success"):
                ...
                audit_info["audit_end_date"] = DateTime().strftime(AUDIT_DATE_FORMAT)
