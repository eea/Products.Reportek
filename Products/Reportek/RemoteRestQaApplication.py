...
manage_addRemoteRESTQAApplicationForm = PageTemplateFile(
    "zpt/remote/application_add_rest_qa", globals()
)


def manage_addRemoteRESTQAApplication(
    self, id="", title="", RemoteServer="", token="", app_name="", REQUEST=None
):
...
class RemoteRestQaApplication(BaseRemoteApplication):
    ...
    as an additional property - app_name - contaning a dictionary like::

      {
          ...
          "getResult": {jobID: {
              code, retries_left, last_error, next_run, fileURL}},
      }
    ...
    def __call__(self, workitem_id, REQUEST=None):
        ...
        if not no_of_files:
            self.__manageAutomaticProperty(
                p_workitem_id=workitem_id, p_analyze={"code": 2}
            )
    ...
    def callApplication(self, workitem_id, REQUEST=None):
        ...
        elif l_wk_prop["analyze"]["code"] == -2 and self._local_scripts_done(
            l_wk_prop.get("localQA")
        ):
    ...
    def _addFeedback(self, file_id, result, workitem, script_id):
        ...
        feedback_title = "{0} result for file {1}: {2}".format(
            self.app_name, file_id, script_title
        )
    ...
    def _runLocalQAScripts(self, workitem):
        ...
        xmls = (
            x
            for x in self.aq_parent.objectValues(Document.meta_type)
    ...
    def __analyzeDocuments(self, p_workitem_id):
        ...
        try:
            ...
            if len(data) == 0:
                self.__manageAutomaticProperty(
                    p_workitem_id=p_workitem_id, p_analyze={"code": 2}
                )
                return 0
            ...
            job_ret = (
                self.nJobRetries
                if getattr(self, "nJobRetries", None)
                else self.nRetries
            )
            for entry in data:
                l_job = entry["jobId"]
        ...
        # in this error type
            ...
            if l_nRetries == 0:
                ...
                self.__manageAutomaticProperty(
                    p_workitem_id=p_workitem_id,
                    p_analyze={"code": -2, "last_error": err},
                )
            else:
                next_run = DateTime(
    ...
    manage_settings_html = PageTemplateFile(
        "zpt/remote/application_edit_rest_qa", globals()
    )
        ...
        try:
            ...
            l_workitem.addEvent(
                "#{} job cancelation triggered by: {} - {}: {}".format(
                    job_id, username, res.status_code, message
                )
            )
    ...
    def runQAScript(self, p_file_url, p_script_id):
        """ """
        url = "qajobs"
        data = {"sourceUrl": p_file_url, "scriptId": p_script_id}
