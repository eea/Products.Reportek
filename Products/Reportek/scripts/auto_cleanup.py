...
def do_cleanup(container, c_type, prefix, threshold):
    objs = [
        obj for obj in container.objectValues(c_type) if obj.getId().startswith(prefix)
    ]
    ...
    if len(objs) > threshold:
        ...
        for d_id in delete_ids:
            print(
                "Removing {}: {}".format(
                    c_type, "/".join([container.absolute_url(1), d_id])
                )
            )
...
def main():
    ...
    parser.add_argument("--container", help="Folder path", dest="container")
    parser.add_argument(
        "--recursive",
        help="Recurse through all subfolderish containers",
        dest="recursive",
    )
    parser.add_argument("--c_type", help="Content type", dest="c_type")
    parser.add_argument("--prefix", help="Content type id prefix", dest="prefix")
    parser.add_argument(
        "--threshold",
    ...
    if args.recursive.lower() in ["true", "1", "on"]:
        folderish = [obj for obj in container.objectValues() if IFolder.providedBy(obj)]
