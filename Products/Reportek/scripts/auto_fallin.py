...
if SCHEDULE_PERIOD == "weekly":
    if int(math.ceil(now.earliestTime() - SCHEDULE_START.earliestTime())) % 7 == 0:
        should_run = True
        print(
            "Weekly run condition evaluates to: {}".format(
    ...
    if (
        SCHEDULE_START.month() != now.month()
        or (
            SCHEDULE_START.month() == now.month()
...
def get_envelopes(catalog, df_uris, act_from, act_to):
    ...
    for brain in brains:
        ...
        results["/".join(env.getPhysicalPath())] = {"envelope": env, "wk": wk}
    return results
...
def main():
    ...
    like so:
    bin/instance run bin/auto_fallin --obligation <obligation_id>\
    --act_from <ActivityID> --act_to <ActivityID> --env_year 2017
    """
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--obligations",
        help="Obligation(s), e.g. 673 or\
    ...
    parser.add_argument("--workflow", help="Workflow of the envelopes", dest="act_wf")
    parser.add_argument(
        "--env_year_offset",
        help="Envelope year offset, e.g. +1, meaning current year + 1,\
            default=+0",
    ...
    if should_run:
        ...
        obls = [
            df_prefix.format(obl) if not obl.startswith("http") else obl for obl in obls
        ]
        site = get_zope_site()
        ...
        try:
            for key, value in results.items():
                ...
                if (
                    str(env.year) == str(env_year)
                    and args.act_wf == env_wf.id
                    and activity
                ):
                    try:
                        ...
                        print(
                            "{} moved from {} to {}".format(
                                env.absolute_url(1), args.act_from, args.act_to
                            )
                        )
