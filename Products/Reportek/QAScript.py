...
manage_addQAScriptForm = PageTemplateFile(
    os.path.join(package_home(globals()), "zpt/qa/script_add.zpt")
)


def manage_addQAScript(
    self,
    id,
    title="",
    description="",
...
class QAScript(SimpleItem):
    ...
    manage_settings_html = PageTemplateFile(
        os.path.join(package_home(globals()), "zpt/qa/script_edit.zpt")
    )
