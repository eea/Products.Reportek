...
class RemoteRESTApplication(SimpleItem):
    ...
    manage_settings_html = PageTemplateFile("zpt/remote/rest_edit", globals())
    ...
    def __call__(self, workitem_id, REQUEST=None):
        ...
        if resp.status_code == 200:
            ...
            try:
                ...
                workitem.addEvent(
                    "%s job request for %s response is not json."
            ...
            if data and "jobId" in data:
                ...
                workitem.addEvent(
                    "%s job request for %s response is invalid."
                    % (self.app_name, envelope_url)
                )
        else:
            workitem.addEvent(
    ...
    def callApplication(self, workitem_id, REQUEST):
        ...
        if resp.status_code == 200:
            ...
            try:
                ...
                workitem.addEvent(
                    "%s job id %s for %s output is not json."
                ...
                if job_status == "esriJobSucceeded":
                    ...
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            logger.warning(
                                "Could not fetch results from: %s" % result_url
                            )
                    except KeyError as err:
                        logger.warning("Unable to find %s in JSON." % err)

                    messages = (
                        """The results for this assessment are """
                    ...
                    workitem.addEvent(
                        "%s job id %s for %s is still running."
                        % (self.app_name, jobid, envelope_url)
                    )
                    self.__decrease_retries(workitem, REQUEST)
                else:
                    workitem.addEvent(
                        "%s job id %s for %s has status %s."
                        % (self.app_name, jobid, envelope_url, job_status)
                    )
                    self.__decrease_retries(workitem, REQUEST)
            else:
                workitem.addEvent(
                    "%s job id %s for %s output is invalid."
                    % (self.app_name, jobid, envelope_url)
                )
                self.__finish(workitem_id, REQUEST)
        else:
