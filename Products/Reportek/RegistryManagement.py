...
class FGASRegistryAPI(BaseRegistryAPI):
    ...
    def get_registry_companies(self, detailed=False):
        ...
        if detailed:
            page = 'list'
        url_prefix = self.baseUrl + '/undertaking/fgas/'
        url = url_prefix + page
    ...
    def get_company_details(self, company_id):
        url = self.baseUrl + '/undertaking/fgas/{0}/details'.format(company_id)
        response = self.do_api_request(url, headers={'Authorization':self.token})
        if response:
            return response.json()
    ...
    def existsCompany(self, params):
        url = self.baseUrl + '/undertaking/fgas/filter/'
    ...
    def verifyCandidate(self, companyId, userId):
        ...
        api_url = self.baseUrl + "/candidate/fgas/verify/{0}/"
    ...
    def getCandidates(self):
        url = self.baseUrl + '/candidate/fgas/list'
        response = self.do_api_request(url, headers={'Authorization':self.token})
    ...
    def unverifyCompany(self, companyId, userId):
        ...
        if co:
            url = self.baseUrl + '/candidate/fgas/unverify/{0}/'.format(companyId)
            data = {'user': userId}
            response = self.do_api_request(url,
                    data=data,
                    method="post",
    ...
    def updateCompanyStatus(self, company_id, status):
        url = self.baseUrl + '/undertaking/fgas/{0}/statusupdate'.format(company_id)
        data = {'status': status}
    ...
    def getCompaniesExcelExport(self):
        url = self.baseUrl + '/export/undertaking/fgas'
        response = self.do_api_request(url, headers={'Authorization':self.token})
