...
CACHE_PROXY = (
    ("{}:{}".format(CACHE_PURGING_PROXY, CACHE_PURGING_PROXY_PORT),)
    if CACHE_PURGING_PROXY and CACHE_PURGING_PROXY_PORT
    else ()
)
# ('http://example.com:80`, 'http://www.example.com:80',)
CACHE_DOMAINS = (
    (
        "http://{}:80".format(CACHE_PURGING_DOMAIN),
        "https://{}:443".format(CACHE_PURGING_DOMAIN),
    )
    if CACHE_PURGING_DOMAIN
    else ()
)
...
def registry_setup(registry):
    ...
    settings.operationMapping["reportek_contentTypes"] = (
        "Products.Reportek.caching.moderateCaching"  # noqa: E501
    )

    # Turn on Cache Purging Settings
    registry["plone.cachepurging.interfaces.ICachePurgingSettings.enabled"] = (
        CACHE_PURGING_ENABLED  # noqa: E501
    )
