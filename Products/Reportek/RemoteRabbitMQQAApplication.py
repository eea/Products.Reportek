...
manage_addRRMQQAApplicationForm = PageTemplateFile(
    "zpt/remote/application_add_rabbitmq_qa", globals()
)


def manage_addRRMQQAApplication(
    self,
    id="",
    title="",
    qarequests="",
    ...
    ob = RemoteRabbitMQQAApplication(
        id, title, qarequests, qajobs, qadeadletter, qaserver, token, app_name
    )
...
class RemoteRabbitMQQAApplication(BaseRemoteApplication):
    ...
    def do_request(self, workitem_id):
        ...
        data = {"envelopeUrl": l_envelope.absolute_url(), "uuid": str(wk.UUID)}
    ...
    def handle_result(self, wk, payload):
        ...
        if job_result:
            ...
            if r_files:
                ...
                fb_title = "".join(
                    [self.app_name, l_filename, payload.get("scriptTitle")]
                )
    ...
    def callApplication(self, workitem_id, REQUEST=None):
        ...
        if self.REQUEST and self.REQUEST.get("BODY"):
            try:
                ...
                feedback_log.exception("Unable to parse payload: {}".format(str(e)))

        if payload and self.check_uuid(workitem_id, payload.get("uuid")):
            if payload.get("errorMessage"):
                wk.addEvent(
                    "{} process failed due to:{}".format(
            ...
            handled = len(
                [
                    j
                    for j in l_wk_prop["jobs_summary"]
    ...
    manage_settings_html = PageTemplateFile(
        "zpt/remote/application_edit_rabbitmq_qa", globals()
    )
