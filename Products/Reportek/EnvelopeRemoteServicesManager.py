...
class EnvelopeRemoteServicesManager:
    ...
    def hasFilesForSchema(self, p_schema_url):
        ...
        l_list = [
            x
            for x in self.objectValues("Report Document")
            if x.xml_schema_location == p_schema_url
        ]
        return len(l_list)
    ...
    def getFilesForSchema(self, schema_uri):
        ...
        return [
            doc
            for doc in self.objectValues(Document.meta_type)
    ...
    def getDocumentsForRemoteService(self):
        ...
        for docu in self.objectValues("Report Document"):
            if docu.xml_schema_location:
                ...
                schema_locations.append(docu.xml_schema_location)
                for schema in schema_locations:
                    if (
                        docu.content_type == "text/xml"
                        and schema
                        and (schema in l_valid_schemas or not l_valid_schemas)
                    ):
                        l_key = str(schema)
                        if l_key in l_res:
                            l_res[l_key].append(
        ...
        try:
            if file_id in self.objectIds():
                ...
                self.manage_addDocument(
                    id=file_id, title=title, file=file, content_type="text/xml"
                )
                l_file = self._getOb(file_id)
    ...
    def getValidXMLSchemas(self, web_form_only=False):
        ...
        return mappings_c.getSchemasForDataflows(
    ...
    def get_webform_for_schema(self, schema):
        mappings_c = self.getDataflowMappingsContainer()
        wf_cust = mappings_c.get_webform_url_for_schema(
    ...
    def getWebQ_BeforeEditForm_URL(self, schema=None, custom_params=False):
        ...
        attributes = {
            ...
            "countrycode": (
                self.getCountryCode(self.country)
